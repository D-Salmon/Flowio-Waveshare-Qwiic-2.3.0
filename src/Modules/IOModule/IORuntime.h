...
static inline void setIoEndpointAnalogDiagnostic(DataStore& ds, uint8_t idx, float inputValue,
                                                 int16_t rawBinary, bool rawBinaryValid,
                                                 bool sampleValid, uint32_t tsMs)
{
    if (idx >= IO_MAX_ENDPOINTS) return;
    IOEndpointRuntime& ep = ds.dataMutable().io.endpoints[idx];
    ep.analogSampleValid = sampleValid;
    ep.analogRawBinaryValid = rawBinaryValid;
    ep.analogInputValue = inputValue;
    ep.analogRawBinary = rawBinary;
    ep.analogSampleTimestampMs = tsMs;
}
