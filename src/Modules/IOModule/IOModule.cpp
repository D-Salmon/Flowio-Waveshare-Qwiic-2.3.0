...
void IOModule::invalidateAnalogSlot_(AnalogSlot& slot, uint32_t nowMs)
{
    ...
    if (dataStore_) {
        ...
        if (endpointIndexFromId_(slot.def.id, rtIdx)) {
            ...
            setIoEndpointAnalogDiagnostic(*dataStore_, rtIdx, 0.0f, 0, false, false, nowMs);
        }
    }
...
bool IOModule::processAnalogDefinition_(uint8_t idx, uint32_t nowMs)
{
    ...
    if (dataStore_) {
        uint8_t rtIdx = 0;
        if (endpointIndexFromId_(slot.def.id, rtIdx)) {
            setIoEndpointAnalogDiagnostic(*dataStore_, rtIdx, raw, rawBinary, sample.hasRaw, true, nowMs);
        }
    }
...
bool IOModule::resolveDsBusAddress_(IOneWireTemperatureBus* bus, const char* runtimeKey, uint8_t index, uint8_t outAddr[8])
{
    ...
    if (bus->deviceCount() <= index) return false;
    if (!bus->getAddress(index, outAddr)) return false;
...
bool IOModule::configureRuntime_()
{
    ...
    const bool needDs2484 = useDs2484_ && (needAnalogSource[IO_SRC_DS18_WATER] || needAnalogSource[IO_SRC_DS18_AIR]);
    const bool needI2c =
        needDs2484 ||
    ...
    if (needI2c) {
        ...
        if (needDs2484) {
            ...
            LOGI("DS2484 probe 0x%02X: %s", ds2484Address_, ds2484Present ? "found" : "not found");
