...
AlarmCondState PoolLogicModule::condPhTankLowStatic_(void* ctx, uint32_t)
{
    PoolLogicModule* self = static_cast<PoolLogicModule*>(ctx);
    if (!self || !self->enabled_) return AlarmCondState::False;
    if (!self->tankLevelMonitoringEnabled_) return AlarmCondState::False;
...
AlarmCondState PoolLogicModule::condFiltrationContactorMismatchStatic_(void* ctx, uint32_t)
{
    ...
    return self->condContactorMismatch_(self->filtrationFeedbackMonitoringEnabled_,
                                        self->filtrationFeedbackIoId_,
                                        self->filtrationFsm_);
}

AlarmCondState PoolLogicModule::condChlorineGeneratorContactorMismatchStatic_(void* ctx, uint32_t)
{
    PoolLogicModule* self = static_cast<PoolLogicModule*>(ctx);
    if (!self) return AlarmCondState::Unknown;
    return self->condContactorMismatch_(self->chlorineGeneratorFeedbackMonitoringEnabled_,
                                        self->chlorineGeneratorFeedbackIoId_,
                                        self->swgFsm_);
}

AlarmCondState PoolLogicModule::condContactorMismatch_(bool monitoringEnabled,
                                                       IoId feedbackIoId,
                                                       const DeviceFsm& device) const
{
    if (!enabled_ || !monitoringEnabled) return AlarmCondState::False;
    if (!device.known) return AlarmCondState::Unknown;

    bool feedbackOn = false;
    if (!loadDigitalSensor_(feedbackIoId, feedbackOn)) return AlarmCondState::Unknown;
    return (feedbackOn != device.on) ? AlarmCondState::True : AlarmCondState::False;
}
...
void PoolLogicModule::runControlLoop_(uint32_t nowMs)
{
    ...
    if (alarmSvc_ && alarmSvc_->isActive) {
        ...
        const bool phTankLowAlarm =
            tankLevelMonitoringEnabled_ && alarmSvc_->isActive(alarmSvc_->ctx, AlarmId::PoolPhTankLow);
        const bool chlorineTankLowAlarm =
            tankLevelMonitoringEnabled_ && alarmSvc_->isActive(alarmSvc_->ctx, AlarmId::PoolChlorineTankLow);
    ...
    const bool filtrationFeedbackFault =
        filtrationFeedbackMonitoringEnabled_ && alarmSvc_ && alarmSvc_->isActive &&
        alarmSvc_->isActive(alarmSvc_->ctx, AlarmId::PoolFiltrationContactorMismatch);
    if (filtrationFeedbackFault) {
        // Electrolysis must not remain energized when the filtration power
        // contactor does not match its commanded state.
        swgDesired = false;
    }
