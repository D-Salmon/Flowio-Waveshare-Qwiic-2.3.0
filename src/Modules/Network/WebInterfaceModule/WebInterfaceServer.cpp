                                             ...
                                             uint32_t& retryAfterSeconds)
{
    ...
    portENTER_CRITICAL(&webAuthThrottleMux_);
    if (webAuthGlobalBlockedUntilMs_ != 0U &&
        (int32_t)(webAuthGlobalBlockedUntilMs_ - now) > 0) {
        const uint32_t remainingMs = webAuthGlobalBlockedUntilMs_ - now;
        retryAfterSeconds = (remainingMs + 999U) / 1000U;
        limited = true;
    } else if (webAuthGlobalBlockedUntilMs_ != 0U) {
        webAuthGlobalBlockedUntilMs_ = 0U;
        webAuthGlobalWindowStartMs_ = now;
        webAuthGlobalFailures_ = 0U;
    }
    for (uint8_t i = 0; i < kWebAuthThrottleSlots; ++i) {
        ...
        if (entry.blockedUntilMs != 0U && (int32_t)(entry.blockedUntilMs - now) > 0) {
            ...
            if (sourceRetryAfter > retryAfterSeconds) {
                retryAfterSeconds = sourceRetryAfter;
            }
            limited = true;
...
void WebInterfaceModule::noteWebAuthFailure_(AsyncWebServerRequest* request)
{
    ...
    bool newlyGloballyBlocked = false;
    ...
    if (webAuthGlobalWindowStartMs_ == 0U ||
        (now - webAuthGlobalWindowStartMs_) >= kWebAuthWindowMs) {
        webAuthGlobalWindowStartMs_ = now;
        webAuthGlobalFailures_ = 0U;
    }
    if (webAuthGlobalFailures_ < UINT8_MAX) ++webAuthGlobalFailures_;
    if (webAuthGlobalFailures_ >= kWebAuthGlobalMaxFailures &&
        (webAuthGlobalBlockedUntilMs_ == 0U ||
         (int32_t)(webAuthGlobalBlockedUntilMs_ - now) <= 0)) {
        webAuthGlobalBlockedUntilMs_ = now + kWebAuthGlobalBlockMs;
        newlyGloballyBlocked = true;
    }
    portEXIT_CRITICAL(&webAuthThrottleMux_);
