...
bool isMaskedConfigKey_(const char* key)
{
    return key &&
           (strcmp(key, "pass") == 0 ||
            strcmp(key, "token") == 0 ||
...
size_t jsonEncodedStringLength_(const char* value)
{
    size_t len = 2U;  // opening and closing quotes
    if (!value) return len;

    const uint8_t* cursor = reinterpret_cast<const uint8_t*>(value);
    while (*cursor != 0U) {
        size_t encodedLen = 1U;
        switch (*cursor) {
            case '\"':
            case '\\':
            case '\b':
            ...
            case '\t':
                ...
                break;
            default:
                if (*cursor < 0x20U) encodedLen = 6U;
                break;
        }
        if (len > SIZE_MAX - encodedLen) return SIZE_MAX;
        len += encodedLen;
        ++cursor;
    }
    return len;
...
void writeJsonEncodedString_(char* out, size_t& pos, const char* value)
{
    out[pos++] = '\"';
    if (value) {
        const uint8_t* cursor = reinterpret_cast<const uint8_t*>(value);
            ...
            switch (c) {
                ...
                default:
                    if (c < 0x20U) {
                        ...
                        out[pos++] = '0';
                        out[pos++] = '0';
                        out[pos++] = kJsonHexDigits[(c >> 4U) & 0x0FU];
                        out[pos++] = kJsonHexDigits[c & 0x0FU];
                    } else {
...
bool formatConfigJsonValue_(const ConfigMeta& meta,
                            bool maskSecrets,
                            char* scalar,
                            size_t scalarLen,
                            const char*& value,
                            bool& quoted)
{
    value = scalar;
    quoted = false;
    if (!scalar || scalarLen < 5U) return false;

    if (!meta.valuePtr) {
        memcpy(scalar, "null", 5U);
        return true;
    }
    ...
    switch (meta.type) {
        case ConfigType::Int32:
            n = snprintf(scalar, scalarLen, "%ld", (long)*(int32_t*)meta.valuePtr);
            break;
        case ConfigType::UInt16:
            n = snprintf(scalar, scalarLen, "%u", (unsigned)*(uint16_t*)meta.valuePtr);
            break;
        case ConfigType::UInt8:
            n = snprintf(scalar, scalarLen, "%u", (unsigned)*(uint8_t*)meta.valuePtr);
            break;
        case ConfigType::Bool:
            value = (*(bool*)meta.valuePtr) ? "true" : "false";
            return true;
        case ConfigType::Float:
            if (!isfinite(*(float*)meta.valuePtr)) {
                memcpy(scalar, "null", 5U);
                return true;
            }
            n = snprintf(scalar, scalarLen, "%.3f", (double)*(float*)meta.valuePtr);
            break;
        case ConfigType::Double:
            if (!isfinite(*(double*)meta.valuePtr)) {
                memcpy(scalar, "null", 5U);
                return true;
            }
            n = snprintf(scalar, scalarLen, "%.6f", *(double*)meta.valuePtr);
            break;
        case ConfigType::CharArray:
            value = (maskSecrets && isMaskedConfigKey_(meta.name))
                ? "***"
                : (const char*)meta.valuePtr;
            quoted = true;
            return true;
        default:
            memcpy(scalar, "null", 5U);
    ...
    return n > 0 && (size_t)n < scalarLen;
}

bool appendConfigJsonEntry_(char* out,
                            size_t outLen,
                            size_t& pos,
                            ...
                            bool maskSecrets)
{
    ...
    bool quoted = false;
    if (!formatConfigJsonValue_(meta,
                                maskSecrets,
                                scalar,
                                sizeof(scalar),
                                value,
    ...
    const char* key = meta.name ? meta.name : "";
    const size_t keyLen = jsonEncodedStringLength_(key);
    const size_t valueLen = quoted ? jsonEncodedStringLength_(value) : strlen(value);
    if (keyLen == SIZE_MAX || valueLen == SIZE_MAX) return false;

    const size_t commaLen = withComma ? 1U : 0U;
    if (keyLen > SIZE_MAX - commaLen ||
        keyLen + commaLen > SIZE_MAX - 1U ||
        valueLen > SIZE_MAX - commaLen - keyLen - 1U) {
        return false;
    }
    const size_t required = commaLen + keyLen + 1U + valueLen;

    // Keep room for the object closing brace and the terminating NUL.
    if (pos > outLen || required > outLen - pos ||
        outLen - pos - required < 2U) {
        return false;
    }

    if (withComma) out[pos++] = ',';
    writeJsonEncodedString_(out, pos, key);
    out[pos++] = ':';
    if (quoted) {
        writeJsonEncodedString_(out, pos, value);
    } else {
...
void ConfigStore::toJson(char* out, size_t outLen) const
{
    ...
    for (uint16_t i = 0; i < _metaCount; ++i) {
        ...
        if (!appendConfigJsonEntry_(out, outLen, pos, m, any, false)) break;
        any = true;
    }
                               ...
                               bool maskSecrets) const
{
    ...
    for (uint16_t i = 0; i < _metaCount; ++i) {
        ...
        if (!appendConfigJsonEntry_(out, outLen, pos, m, any, maskSecrets)) {
            truncatedLocal = true;
            break;
        }
        any = true;
